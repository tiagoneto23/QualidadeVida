    ...
    with open(os.path.join(diretorio_resultados, 'analise_correlacao.txt'), 'w') as f:
        ...
        for col1, col2, corr in correlacoes_ordenadas:
            ...
            if abs(corr) > 0.7:
                if corr > 0:
                    f.write(
                        f"  Interpretação: Forte correlação positiva - quando {col1} aumenta, {col2} tende a aumentar também.\n")
                else:
                    f.write(
                        f"  Interpretação: Forte correlação negativa - quando {col1} aumenta, {col2} tende a diminuir.\n")
            elif abs(corr) > 0.5:
                if corr > 0:
...
def analisar_regressoes(df_integrado):
    ...
    with open(os.path.join(diretorio_resultados, 'analise_regressao.txt'), 'w') as f:
        ...
        for resultado in resultados_regressao:
            ...
            f.write(
                f"Equação da reta: {resultado['var_y']} = {resultado['slope']:.4f} * {resultado['var_x']} + {resultado['intercept']:.4f}\n")
            ...
            if resultado['r_squared'] > 0.7:
                ...
                f.write(
                    f"- O modelo explica apenas uma pequena parte da variação em {resultado['var_y']} (R² < 0.5).\n")

            if resultado['slope'] > 0:
                f.write(
                    f"- Para cada unidade de aumento em {resultado['var_x']}, {resultado['var_y']} aumenta em média {resultado['slope']:.4f} unidades.\n")
            else:
                f.write(
                    f"- Para cada unidade de aumento em {resultado['var_x']}, {resultado['var_y']} diminui em média {abs(resultado['slope']):.4f} unidades.\n")
